...
## In This Project
This project has one main Python module `GamesHub.py` which runs the Flask application. A SQL database is created using the `Data_Setup.py` module and you can populate the database with test data using `database_init.py`.
...
7. Setup application database `python /GamesHub/Data_Setup.py`
8. *Insert sample data `python /GamesHub/database_init.py`
9. Run application using `python /GamesHub/GamesHub.py`
...
game Categories JSON: `/gamekStore/gameCategories/JSON`
    - Displays all game categories
All game Editions: `/gameStore/editions/JSON`
	- Displays all game Models

game Edition JSON: `/gamekStore/<path:game_name>/editions/JSON`
    - Displays game models for a specific game category

game Category Edition JSON: `/gameStore/<path:gamek_name>/<path:edition_name>/JSON`
